...
export default function PageTitle({
  ...
  return (
    <div className="flex items-end justify-start w-full px-4 sm:px-0">
      <h1 className="text-[26px] sm:text-[36px] font-600 flex items-end w-full px-0 whitespace-nowrap">
        {title}
      </h1>
      <div className={`
        ${!showButton ? "hidden" : "flex items-center"}
        ${buttonType === "hover" ? "max-md:hidden" : ""}
      `}>
        {renderButton()}
      </div>
    </div>
